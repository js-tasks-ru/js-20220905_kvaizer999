...
export default class ColumnChart {
  ...
  constructor({
    ...
    this.link = link;
    this.url = `${ColumnChart.BACKEND_URL}/${url}`;
    this.to = range.to;
  ...
  async getData() {
    const requestUrl = `${this.url}?from=${this.from.toISOString()}&to=${this.to.toISOString()}`;
    try {
      const response = await fetch(requestUrl);
      if (response.ok) {
        const responseObj = await response.json();
        this.data = Object.values(responseObj);
        this.value = Object.values(this.data).reduce((acc, curr) => acc += curr, 0);
        return responseObj;
      }
    } catch (err) {
  ...
  async update(start = this.from, end = this.to) {
    ...
    return response; //для теста 'should load data correctly'
  }
